...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ajax: assetsPath + 'json/employee-list.json', // JSON file to add data
      columns: [
  ...
  render: function (data, type, full, meta) {
    ...
    if ($image && $image.trim() !== '') {
      // For Avatar image
      var $output = '<img src="' + assetsPath + 'uploads/admin-avatar/' + $image + '" alt="Avatar" class="rounded-circle">';
    } else {
      // For Avatar badge
...
,{
      ...
      order: [[1, 'desc']], // Order by fullname
      dom:
        '<"row me-2"' +
        '<"col-md-2"<"me-3"l>>' +
    ...
    initComplete: function () {
      ...
      this.api()
        .columns(2) // Changed column index to 2
        .every(function () {
          var column = this;
          var select = $(
            ...
            .sort()
            .each(function (d, j) {
              if (statusObj[d]) { // Check if statusObj[d] is defined
                select.append(
                  '<option value="' +
                    statusObj[d].title +
  ...
  this.api()
  .columns(3) // Changed column index to 3
  .every(function () {
    var column = this;
    var select = $(
